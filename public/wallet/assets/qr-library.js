 ...
 * Adapted from the locally installed qrcode-terminal distribution.
 */
...
function require(id){if(cache[id])return cache[id].exports;if(!modules[id])throw new Error('Module '+id);var m={exports:{}};cache[id]=m;modules[id](require,m,m.exports);return m.exports;}
global.AldreesQR=function(text){if(!/^[\x00-\x7F]*$/.test(text))throw new Error('The URL must be percent-encoded');var Q=require('./index');var levels=require('./QRErrorCorrectLevel');var q=new Q(-1,levels.M);q.addData(text);q.make();return q;};})(window);
